...
$$
\frac{\partial E_{Y|X}[(L_\tau(Y,f(X)))|X=x]}{\partial f(x)} = \int_{min_y}^{f(x)}  (1-\tau) dF_Y(y) + \int_{f(x)}^{max_y}  -\tau dF_Y(y)=F_Y(f(x)) -\int_{min_y}^{f(x)}  \tau dF_Y(y) + \int_{f(x)}^{max_y}  -\tau dF_Y(y)=
\\
...
$$
\textrm{we can see that an p grows the fraction of observations outsde the ball increaces}
$$
