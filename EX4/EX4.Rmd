...
heart= heart[,-5] # the whole column is NAs
...
model_logistic %>% compile(loss = 'mse',optimizer = optimizer_rmsprop(),metrics = c('accuracy'))
...
paste("the test set preformance is:" , round((tbl[2,2] + tbl[1,1])/sum(tbl),4))
...
model_lr %>% compile(loss = 'mse',optimizer = optimizer_rmsprop(),metrics = c('accuracy'))
...
phat_logit = predict(mod.lr, newdata = data.frame(x=x_test,y=y_test))
table(phat_logit>0.5, y_test) # 2*2 table
```

* almost the same results, we are probably near the optima and need to reduce the learning rate
...
model_3_layers %>% 
  layer_dense(units = 2, activation = 'relu', input_shape = c(dim(x_train)[2]))%>% 
...
```{r message=FALSE, warning=FALSE}
model_3_layers %>% compile(loss = 'mse',optimizer = optimizer_rmsprop(),metrics = c('accuracy'))
...
```{r}
phat_NN_1_sig = predict_proba(model_3_layers,x_test, batch_size = NULL, verbose = 0, steps = NULL)
summary(phat_NN_1_sig)
tbl = table(phat_NN_1_sig>0.5, y_test) # 2*2 table
paste("the test set preformance is:" , round((tbl[2,2] + tbl[1,1])/sum(tbl),4))
```
